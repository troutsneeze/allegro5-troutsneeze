...
static void setup_gl(ALLEGRO_DISPLAY *d)
{
   ...
   ALLEGRO_DISPLAY_OSX_WIN* dpy = (ALLEGRO_DISPLAY_OSX_WIN*) d;
   //[dpy->ctx update];
   [dpy->ctx performSelectorOnMainThread: @selector(update) withObject: nil waitUntilDone: NO];
}
...
static ALLEGRO_DISPLAY* create_display_fs(int w, int h)
{
   ...
   //[context update];
   [context performSelectorOnMainThread: @selector(update) withObject: nil waitUntilDone: NO];
   [window_view release];
